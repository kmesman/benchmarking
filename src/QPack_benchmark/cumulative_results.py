...
sum_data = [0]*len(mean_data["circuit\nexecution"][0])
for key, color in zip(raw_data.keys(), colors):
    md = mean_data[key]
    if key != "total":
        rd = raw_data[key]
        plot_data = [d1+sum_data[int(d0-5)] for d0, d1 in zip(rd[0], rd[1])]
        plt.plot(rd[0],plot_data,'.', color=color, alpha=0.3)
        
        sum_data = list(map(lambda a,b: a+b, sum_data, md[1]))
        plt.plot(md[0], sum_data,'-', color=color, label=key)


plt.ylabel("time [s]")
plt.xlabel("(virtual) qubits")
plt.title("Cumulative average runtime per iteration (2000 shots)")
plt.xticks(range(5,23,1))
plt.legend()
#ax.set_yscale('log')
plt.savefig("result_images/Cumulative_results_{}_qubits.pdf".format(qubits))
