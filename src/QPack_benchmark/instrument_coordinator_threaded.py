...
class InstrumentCoordinator(qcodes_base.Instrument):
    ...
    def prepare(
        ...
        def threaded_prepare(instrument_name, args):
            self.get_component(
            base.instrument_to_component_name(instrument_name)
            ).prepare(args)
        """    
        # This runs the prepare method in threads, this way all modules will be configured in parallel.
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(compiled_instructions.items())) as executor:
            executor.map(threaded_prepare, compiled_instructions.items())
        threading.Barrier(len(compiled_instructions.items()))
        
        """
        """
        threads = list()
        for instrument_name, args in compiled_instructions.items():
            x = threading.Thread(target=threaded_prepare, args=(instrument_name, args))
            threads.append(x)
            x.start()
    ...
    def start(self) -> None:
        ...
        """
                ...
                #threaded start, probably unnecessary
        def threaded_start(instrument_name):
            instrument = self.find_instrument(instrument_name)
            instrument.start()
            
        threads = list()
        for instrument_name in self.components():
            x = threading.Thread(target=threaded_start, args=(instrument_name))
            threads.append(x)
            x.start()
        
        for index, thread in enumerate(threads):
            print("Main    : before joining -start- thread {}.".format(index))
            thread.join()
            print("Main    : -start- thread {} done".format(index))
        """
